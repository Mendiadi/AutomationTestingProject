...
@pytest.mark.usefixtures("author_setup1")
...
class TestAuthorPage:
    ...
    def test_validate_books(self, main_page, api, data):
        for i in range(3):
            api.books.post_books(data.generate_book(authorid=self.author1.id))
        books_api = api.books.find_book_by_author_id(authorId=self.author1.id)
        authors_page = main_page.click_authors_btn()
        authors_page.reload()
        author = authors_page.find_author_by_name(self.author1)
        ...
        for book_title, book_author in books_ui_names_author:
            ...
            assert self.author1.name in book_author

    @log_name
    @allure.title("validate headline show name")
    def test_validate_name_headline(self, api, data, main_page):
        authors_page = main_page.click_authors_btn()
        author = authors_page.find_author_by_name(self.author1.name)
        author_page = authors_page.go_to_author(author)
        assert self.author1.name == author_page.get_headline()
    ...
    def test_data_updated_in_books(self, api, data, main_page):
        book = api.books.post_books(data.generate_book(authorid=self.author1.id))
        authors_page = main_page.click_authors_btn()
        authors_page.reload()
        author = authors_page.find_author_by_name(self.author1)
        author_page = authors_page.go_to_author(author)
    ...
    def test_validate_map_cordinate(self, main_page, api, data):
        ...
        author = api.authors.get_author_by_id(id=self.author1.id)
